...
def format_schema() -> str:
    """Returns the fixed schema format (Neo4j graph structure)."""
    return """
...
def build_cypher_prompt(
    schema_text: Union[str, Dict[str, Any], None],
    examples: List[Dict[str, str]]
) -> PromptTemplate:
    """Build Cypher query prompt with schema and examples."""

    # Nếu schema không được truyền → dùng schema mặc định
    if schema_text is None:
        schema = format_schema()
    elif isinstance(schema_text, dict):
        schema = json.dumps(schema_text, ensure_ascii=False, indent=2)
    else:
        schema = str(schema_text)

    # Chuẩn hoá examples thành block
    example_texts = []
    for ex in examples:
        if "question" in ex and "cypher" in ex:
            example_texts.append(f"Câu hỏi: {ex['question']}\nCypher:\n{ex['cypher']}")
    example_block = "\n\n".join(example_texts)
...
- Nếu câu hỏi nhắc "Công tác phí trong nước" → map sang Section code='I' và tableIdx=1
